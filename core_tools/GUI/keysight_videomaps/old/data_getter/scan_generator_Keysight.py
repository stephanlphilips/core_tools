...
def construct_1D_scan_fast(gate, swing, n_pt, t_step, biasT_corr, pulse_lib, digitizer, channels = [1,2]):
    """
...
#    print(load_HVI.__path__)
    ...
    my_seq.upload([0])
    print('1d construct')
    return _digitzer_scan_parameter(digitizer, my_seq, pulse_lib, t_step, (n_pt, ), (gate, ), (np.sort(voltages), ), biasT_corr, 500e6, channels = channels)
...
def construct_1D_scan_MOD(gate, swing, n_pt, MOD_gates, freq_start, freq_step , biasT_corr, pulse_lib, digitizer):
    ...
    return _digitzer_scan_parameter(digitizer, my_seq, pulse_lib, t_step, (n_pt, ), (gate, ), (np.sort(voltages), ), biasT_corr, 500e6)



def construct_2D_scan_fast(gate1, swing1, n_pt1, gate2, swing2, n_pt2, t_step, biasT_corr, pulse_lib, digitizer, channels = [1,2]):
    """
    1D fast scan object for V2.
    ...
    #(np.sort(voltages2),)*n_pt1)
    return _digitzer_scan_parameter(digitizer, my_seq, pulse_lib, t_step, (n_pt1, n_pt2), (gate1, gate2), (tuple(voltages1), (tuple(np.sort(voltages2)),)*n_pt1), biasT_corr, 500e6, channels = channels)
...
#        print(self.channels)
        ...
        return data_out #tuple(np.fliplr(dat) for dat in data_out)
