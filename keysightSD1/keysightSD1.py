# Patched version of keysightSD1 version 2.01.50.
# Fixes:
#   * fixed thread-safety of SD_Wave.newFromArrayDouble
#   * improved speed of SD_Wave.newFromArrayDouble
#   * improved speed of SD_AOU.waveformLoadArrayInt16
#   * improved speed of SD_AOU.waveformReLoadArrayInt16
#
# Known bugs:
#   * SD_Wave.newFromArrayInteger does not return a waveform with correct data.
...
class SD_Wave(SD_Object) :
	...
	# class level lock to avoid concurrent calls to SD_Wave_newFromArrayDouble
	_lock = RLock()
	...
	def newFromArrayDouble(self, waveformType, waveformDataA, waveformDataB = None) :
		if len(waveformDataA) > 0 and (waveformDataB is None or len(waveformDataA) == len(waveformDataB)) :
			# use numpy to convert Python array to C-array. It is 100 times faster than expansion to argument list.
			...
			# C-code of version 2.01.50 is not thread safe. Waveform data gets corrupted.
			with SD_Wave._lock:
				self._SD_Object__handle = self._SD_Object__core_dll.SD_Wave_newFromArrayDouble(
						waveformType, waveform_dataA_C._length_, waveform_dataA_C, waveform_dataB_C);
	...
	def newFromArrayDoubleOriginal(self, waveformType, waveformDataA, waveformDataB = None) :
		if len(waveformDataA) > 0 and (waveformDataB is None or len(waveformDataA) == len(waveformDataB)) :
	...
	def newFromArrayInteger(self, waveformType, waveformDataA, waveformDataB = None) :
		''' 
		NOTE SdS: This method is broken. Bug is probably in C code.
		'''
		if len(waveformDataA) > 0 and (waveformDataB is None or len(waveformDataA) == len(waveformDataB)) :
...
class SD_AOU(SD_Module):
	...
	def waveformReLoadArrayInt16(self, waveformType, dataRaw, waveformNumber, paddingMode = 0) :
		if self._SD_Object__handle > 0 :
			if len(dataRaw) > 0 :
				...
				data_C = data_np.ctypes.data_as(POINTER(c_short*len(dataRaw))).contents
				result = self._SD_Object__core_dll.SD_AOU_waveformReLoadArrayInt16(
                        self._SD_Object__handle, waveformType, data_C._length_, data_C, waveformNumber, paddingMode);
				return result
			else :
				return SD_Error.INVALID_VALUE;
		else :
	...
	def waveformReLoadArrayInt16Original(self, waveformType, dataRaw, waveformNumber, paddingMode = 0) :
		if self._SD_Object__handle > 0 :
			if len(dataRaw) > 0 :
	...
	def waveformLoadInt16(self, waveformType, dataRaw, waveformNumber, paddingMode = 0) :
		if self._SD_Object__handle > 0 :
			if len(dataRaw) > 0 :
				data_np = to_numpy_int16(dataRaw)
				data_C = data_np.ctypes.data_as(POINTER(c_short*len(dataRaw))).contents
				return self._SD_Object__core_dll.SD_AOU_waveformLoadArrayInt16(self._SD_Object__handle, waveformType, data_C._length_, data_C, waveformNumber, paddingMode);
			else :
				return SD_Error.INVALID_VALUE;
		else :
			return SD_Error.MODULE_NOT_OPENED;

	def waveformLoadInt16Original(self, waveformType, dataRaw, waveformNumber, paddingMode = 0) :
		if self._SD_Object__handle > 0 :
			if len(dataRaw) > 0 :
				dataC = (c_short * len(dataRaw))(*dataRaw);
