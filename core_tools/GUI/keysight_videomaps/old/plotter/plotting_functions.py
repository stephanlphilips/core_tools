...
class live_plot(live_plot_abs, QThread):
    ...
    def generate_buffers(self):
        ...
        if self._differentiate == True:
            # assume differentation on two axises.
    ...
    def stop(self):
        ...
        while self.plt_finished != True:
            time.sleep(0.005) #5ms interval to make sure gil releases.
...
class _2D_live_plot(live_plot):
    ...
    def init_plot(self):
        for i in range(self.n_plots):
            ...
            img = pg.ImageItem()
            plot_2D.addItem(img)
            plot_2D.setLabel('bottom', self.parameter_getter.setpoint_labels[i][0], self.parameter_getter.setpoint_units[i][0])
            plot_2D.setLabel('left', self.parameter_getter.setpoint_labels[i][1], self.parameter_getter.setpoint_units[i][1])
            self.top_layout.addWidget(plot_2D, 0, i, 1, 1)

            img.translate(-np.abs(self.parameter_getter.setpoints[0][1][0][-1]), -np.abs(self.parameter_getter.setpoints[0][0][-1]))
        ...
        while (self.active == True):
            try:
                ...
                for i in range(self.n_plots):
                    ...
                    self.plot_data[i] = np.transpose(np.sum(self.buffer_data[i], 0)/len(self.buffer_data[i]))
            except:
                print("frame dropped ...")
...
if __name__ == '__main__':
    ...
    if dim == "1D":
        ...
        plot = _1D_live_plot(app, window.frame_plots, window.grid_plots, p ,1,1)
    ...
    if dim == "2D":
        p = construct_2D_scan_fast("test", 100, 200, "test2", 150, 200, t_step=5, biasT_corr= False, pulse_lib=None, digitizer=dig)
        
        plot = _2D_live_plot(app, window.frame_plots, window.grid_plots, p ,1,1)
