...
class Keysight_E8267D(VisaInstrument):
    ...
    def __init__(self, name, address, step_attenuator=False, **kwargs):
        ...
        self.add_parameter('wideband_IQ_modulation_enabled', get_cmd=f'AM:WID:STATe?', set_cmd=f'AM:WID:STATe {{}}', val_mapping=on_off_mapping)
        self.add_parameter('ALC_status', get_cmd=f'ALC:STATe?', set_cmd=f'ALC:STATe {{}}', val_mapping=on_off_mapping )
        self.add_parameter('FM_status', get_cmd=f'FM:STATe?', set_cmd=f'FM:STATe {{}}', val_mapping=on_off_mapping )
        self.add_parameter('FM_source', get_cmd=f'FM:SOURce?', set_cmd=f'FM:SOURce {{}}', get_parser=lambda s: s.strip(), vals=vals.Enum('EXT1', 'EXT2', 'INT', 'INT1', 'INT2') )
        self.add_parameter('FM_deviation', get_cmd=f'FM:DEViation?', set_cmd=f'FM:DEViation {{}}', get_parser=lambda s: str(s)+'MHz')
